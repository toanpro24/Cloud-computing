...
try:
	...
	EDITPRODUCTPARAMS = {'name': 'butter', 'new_price': 3.98}
	r_edit_product = requests.post(url = URLPRODUCTEDIT, data = EDITPRODUCTPARAMS, headers={'Authorization': 'eyJhbGciOiAiSFMyNTYiLCAidHlwIjogIkpXVCJ9.eyJ1c2VybmFtZSI6ICJncmlmZiJ9.b5d00c0a804f967bacfb3a5187d4fd501490f968261511686514de2f21cc5e2b'})
	edit_product_data = r_edit_product.json()

	if edit_product_data['status'] != 3:
		quit()

	r_edit_product = requests.post(url = URLPRODUCTEDIT, data = EDITPRODUCTPARAMS, headers={'Authorization': 'eyJhbGciOiAiSFMyNTYiLCAidHlwIjogIkpXVCJ9.eyJ1c2VybmFtZSI6ICJqbW0ifQ==.02838fbb9275f0c5f5f9b734d984d683be04491cd3a8cf506016c4903bbe8b4f'})
	edit_product_data = r_edit_product.json()

	if edit_product_data['status'] != 1:
		quit()
